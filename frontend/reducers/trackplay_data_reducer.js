...
import { SET_TRACK_DURATION,
         ...
         TRACK_ENDED,
         PLAYER_SEEK,
         WAVE_FORM_SEEK} from '../actions/player_actions.js';
...
export default (state = { progress: {}, duration: {} }, action) => {
  switch (action.type) {
    ...
    case MOVE_TO_PREV_TRACK:
      return _.merge({}, state, { progress: { [action.prevTrackId]: 0,
                                              [action.currentTrackId]: action.currentTrackTimeStamp }});
    case MOVE_TO_NEXT_TRACK:
      return _.merge({}, state, { progress: { [action.nextTrackId]: 0,
                                              [action.currentTrackId]: action.currentTrackTimeStamp}});
    case TRACK_ENDED:
    ...
    case PLAYER_SEEK:
    case WAVE_FORM_SEEK:
      return _.merge({}, state, { progress: { [action.currentTrackId]: action.progress }});
